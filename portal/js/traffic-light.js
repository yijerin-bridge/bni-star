...
// 현재 월 기준 직전 6개월 윈도우 (채점 범위)
function get6MonthWindow() {
  const t = new Date();
  const end   = new Date(t.getFullYear(), t.getMonth(), 0);      // 전월 말일
  const start = new Date(t.getFullYear(), t.getMonth() - 6, 1);  // 6개월 전 1일
  const fmt = d => `${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`;
  return { start: fmt(start), end: fmt(end) };
}
const WIN6 = get6MonthWindow(); // 페이지 로드 시 한 번 계산

// 윈도우 내 실제 회의 주수 (분모)
function windowDenom(memberJoinDate) {
  const effStart = [WIN6.start, CHAPTER_LAUNCH, memberJoinDate || CHAPTER_LAUNCH].sort().reverse()[0];
  return Math.max(WEEKS_ALL.filter(w => w >= effStart && w <= WIN6.end).length, 1);
}

/* ── 채점 함수 (사용자 지정 기준) ── */
/* 채점 함수·calcMemberScore → scoring.js 참고 */
...
function genAIOverview(memberScores) {
  ...
  const mvpParts = [
    ...
    (topVis?.stats?.totVis||0) > 0 ? `🙋 비지터 ${topVis.name}님(${topVis.stats.totVis}명)` : null,
  ].filter(Boolean);
  if (mvpParts.length)
    tips.push({type:'positive',icon:'🏅',text:`이번 기간 MVP — ${mvpParts.join(' · ')} · 미팅에서 박수로 공개 인정해 주세요!`});

  // 전월대비 트래픽라이트 점수 향상
  if (TODAY > WIN6.end) {
    const improved = memberScores.map(m => {
      const effStart = [WIN6.start, CHAPTER_LAUNCH, m.joined_date || CHAPTER_LAUNCH].sort().reverse()[0];
      const prevRecs = m.recs.filter(r => r.week_date >= effStart && r.week_date <= WIN6.end);
      const prevDenom = Math.max(WEEKS_ALL.filter(w => w >= effStart && w <= WIN6.end).length, 1);
